...
import {
  ...
  TEXTOS_PLANNING,
  estadoProyectoTexto,
} from '../../lib/planningCronograma';
import type { PlanPagoPlanning } from '../../lib/generarPdfPlanning';
...
// Mismo rojo que COLOR_RETRASADA en planningCronograma.ts (usado por el PDF) — antes la vista
// previa en pantalla solo distinguía completada/pendiente, así que una fase claramente atrasada
// se veía igual que una pendiente normal aquí, aunque el PDF descargado sí la marcaba en rojo
// (bug real corregido 2026-08-18).
const COLOR_RETRASADA = '#dc2626';
...
export function PlanningPreview({
  ...
  const esRetrasada = (f: FasePreview) => !f.completada && !!f.fecha_fin && f.fecha_fin < hoyISO;
  const hayRetrasadas = fasesConFechas.some(esRetrasada);
    ...
    <div className="documento-papel bg-surface border border-gray-200 rounded-sm overflow-hidden text-[11px] leading-snug">
      ...
      >
        ...
        <div className="text-right shrink-0">
          <p className={`font-bold uppercase tracking-wide ${TAM_TITULO[config.tamanoTitulo]}`}>
            {t.titulo}
          </p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-sm p-2.5 border" style={cardEstilo}>
            ...
            <p className="text-gray-500 text-[10px]">
              {[clienteTelefono, clienteEmail].filter(Boolean).join(' · ')}
            </p>
          </div>
          <div className="rounded-sm p-2.5 border" style={cardEstilo}>
            <p className="font-semibold uppercase text-[9px] mb-1" style={cardLabelEstilo}>
          ...
          <div className="rounded-sm p-2.5 border" style={cardEstilo}>
            ...
            <p className="text-gray-800 text-[10px]">
              {[
                presupuestoNumero,
                presupuestoFecha,
                presupuestoTotal != null && formatearPrecio(presupuestoTotal),
                planPago.length > 0 &&
                  `${t.planPago} : ${planPago.map((p) => `${p.porcentaje}%`).join(' - ')}`,
              ]
                .filter(Boolean)
                .join('   ·   ')}
            </p>
            ...
            <div className="flex items-center justify-between mb-1.5">
              ...
              <div className="flex items-center gap-2.5 text-[8px] text-gray-500">
                ...
                <span className="flex items-center gap-1">
                  <span
                    className="w-2 h-2 rounded-sm inline-block"
                    style={{ backgroundColor: config.colorPendiente }}
                  />
                  {tc.pendiente}
                </span>
                {hayRetrasadas && (
                  <span className="flex items-center gap-1">
                    <span
                      className="w-2 h-2 rounded-sm inline-block"
                      style={{ backgroundColor: COLOR_RETRASADA }}
                    />
                    {tc.retrasada}
                  </span>
                )}
                ...
                return (
                  <div key={grupo.nombre || '_sin_seccion_'} className="flex flex-col gap-1.5">
                    ...
                    {fasesGrupo.map((f, i) => {
                      ...
                      const duracion = Math.max(diasInclusive(f.fecha_inicio!, f.fecha_fin!), 1);
                      return (
                        <div key={i} className="flex items-center gap-2">
                          <span className="w-24 shrink-0 text-[9px] text-gray-600 truncate">
                            {f.nombre}
                          </span>
                          <div className="flex-1 h-2.5 bg-gray-100 rounded-sm relative overflow-hidden">
                            <div
                              ...
                              style={{
                                ...
                                backgroundColor: f.completada
                                  ? config.colorPrimario
                                  : esRetrasada(f)
                                    ? COLOR_RETRASADA
                                    : config.colorPendiente,
                              }}
                            />
                            {hoyEnRango && (
                              <div
                          ...
                          <span className="w-20 shrink-0 text-[8px] text-gray-400 text-right whitespace-nowrap">
                            {fechaPlanningCorta(f.fecha_inicio!)} –{' '}
                            {fechaPlanningCorta(f.fecha_fin!)}
                          </span>
                        </div>
                      );
                    })}
          ...
          <table className="w-full border-collapse text-[9.5px]">
            <thead>
              <tr style={theadEstilo}>
                <th className="text-center px-1.5 py-1 font-medium rounded-tl-sm">{tc.fase}</th>
                <th className="text-center px-1.5 py-1 font-medium">{tc.descripcion}</th>
                <th className="text-center px-1.5 py-1 font-medium">{tc.inicio}</th>
                <th className="text-center px-1.5 py-1 font-medium">{tc.fin}</th>
                <th className="text-center px-1.5 py-1 font-medium">{tc.duracion}</th>
                <th className="text-center px-1.5 py-1 font-medium rounded-tr-sm">{tc.estado}</th>
              </tr>
            </thead>
            <tbody>
              {grupos.map((grupo) => (
                  ...
                  {grupo.nombre && (
                    <tr key={`seccion-${grupo.nombre}`} style={{ backgroundColor: colorClaro }}>
                      ...
                      >
                        ...
                        {grupo.dias != null ? `${grupo.dias} ${tc.dias}` : '—'}
                      </td>
                    </tr>
                  )}
                  {grupo.fases.map((f, i) => (
                    <tr
                      key={`${grupo.nombre}-${i}`}
                      className={[
                        config.tabla.lineas ? 'border-b border-gray-100' : '',
                        config.tabla.filasIntercaladas && i % 2 === 1 ? 'bg-gray-50' : '',
                      ]
                        .filter(Boolean)
                        .join(' ')}
                    >
                      <td className="px-1.5 py-1 font-semibold text-gray-900 text-center text-[9.5px]">
                        {f.nombre}
                      </td>
                      <td className="px-1.5 py-1 text-gray-500">
                        {f.descripcion
                          ? parsearTextoEnriquecido(f.descripcion).map((bloque, idx) => (
                              <p
                                key={idx}
                                className={`${bloque.negrita ? 'font-semibold' : ''} ${bloque.cursiva ? 'italic' : ''}`}
                              >
                                {bloque.texto}
                              </p>
                            ))
                        ...
                        <span
                          ...
                          style={
                            f.completada
                              ...
                              : esRetrasada(f)
                                ? { backgroundColor: COLOR_RETRASADA }
                                : undefined
                          }
                        >
                          {f.completada
