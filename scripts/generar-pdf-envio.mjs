// Genera el PDF de propuesta de envío (email) de un presupuesto a partir de un JSON estructurado.
// Uso: node scripts/generar-pdf-envio.mjs entrada.json salida.pdf
...
if (!inputPath || !outputPath) {
  console.error('Uso: node scripts/generar-pdf-envio.mjs entrada.json salida.pdf');
  process.exit(1);
}
...
let y = cabecera(doc, {
  etiqueta: 'Propuesta de envío · Email',
  titulo: `${d.numero} — ${d.cliente}`,
  meta: `Idioma: ${d.idioma}   ·   Canal: ${d.canal}   ·   Estado: ${d.estado}`,
});
...
// Cabecera de la tarjeta (Para / Asunto)
...
doc.text(`📎 Adjunto: ${d.adjunto}`, MARGEN, y);
doc.setFont('helvetica', 'normal');
doc.setTextColor(...COLOR.gray900);
y += 10;
...
if (d.avisos?.length) {
  for (const aviso of d.avisos) {
    y = cajaNota(doc, y, aviso, aviso.startsWith('CORRECCIÓN') ? 'verde' : 'ambar');
    y += 2;
  }
}

piePaginas(doc, `Presupuesto ${d.numero} · borrador de mensaje, no enviado`);
